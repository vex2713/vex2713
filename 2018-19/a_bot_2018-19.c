...
#pragma config(Motor,  port6,           rightTrack,    tmotorVex393_MC29, openLoop, reversed, encoderPort, I2C_1)
...
//1/22/19 STUFF DAMIAN ADDED FOR ENCODER ON RIGHT MOTOR
...
//	writeDebugStreamLine("enc left:%d",SensorValue[rightEncoder]);
//  writeDebugStreamLine("enc left:%d",clicks);
	...
	while	(SensorValue[rightEncoder]<clicks)
	{
			//go forward for 1/10 second
		motor[rightTrack] = power;
		motor[leftTrack] = power;
		sleep(100);
...
void goBackInches(float inches)
{
	...
	SensorValue[rightEncoder]=0;
	while	(SensorValue[rightEncoder]>=-1*clicks)
	{
			//go back for 1/10 second
		motor[rightTrack] = -1*power;
		motor[leftTrack] = -1*power;
		sleep(100);
...
void turnRightInches(float inches)
{
	int power;
	int clicks;
	clicks=inches*50;
	power=defaultMotorSpeed;
	writeDebugStreamLine("starting turnRightByInches");
	SensorValue[rightEncoder]=0;
	while	(SensorValue[rightEncoder]>=-1*clicks)
	{
			//go back for 1/10 second
		...
		writeDebugStreamLine("ending turnRightInches");

}

void turnLeftInches(float inches)
{
	int power;
	int clicks;
	clicks=inches*50;
	power=defaultMotorSpeed;
	writeDebugStreamLine("starting turnLeftInches");
	SensorValue[rightEncoder]=0;
	while	(SensorValue[rightEncoder]>=1*clicks)
	{
			//go back for 1/10 second
		...
		writeDebugStreamLine("ending turnLeftInches");
