...
#pragma config(Motor,  port3,           fLeft,         tmotorVex393_MC29, openLoop, reversed)
...
#pragma config(Motor,  port7,           Sweeper,       tmotorVex393HighSpeed_MC29, openLoop)
...
task autonomous()
{
	...
	motor[bLeft] = 127;
	motor[bRight] = 127;
	delay(2); //If the ball is not going far enough, increase the parameter, if the ball is going to far, derease it
	motor[fLeft] = 0;
	motor[fRight] = 0;
	motor[bLeft] = 0;
	motor[bRight] = 0;
	motor[sLeft] = 127;
	motor[sRight] = 127;
	wait(2); //If the ball is shooting before the spinner is fast enough, increase the time, if it is shooting after the spinner stopped, decrease the time
	motor[Track] = 127;
}
