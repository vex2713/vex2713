...
#pragma config(Sensor, dgtl2,  LL,             sensorTouch)
...
#pragma config(Motor,  port10,          L3,            tmotorVex393_HBridge, openLoop)
...
task user_b_bot()
{
	...
	*  then hold that position using closed loop control until 6U or 6D are pressed */
	...
	while(1)
	{
		...
		if ((vexRT[Btn5U] == 1) && (SensorValue[LL] == false))
		{
			motor[L1] = 127;
			motor[L2] = 127;
		  motor[L3] = 127;
